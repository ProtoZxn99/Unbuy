...
class items extends CI_Controller{
	...
	$session_data = $this->session->userdata('logged');
        ...
        if(($level==2) || ($level==1)){
            ...
            $this->load->view('statistic/items');
            $this->load->view('foot');
        }else{
    ...
    public function ajax_list() {
        if($this->session->userdata('logged')){
            ...
            foreach ($list->result() as $row) {
                ...
                $q_total = mysql_query("SELECT ifnull(sum(item_order),0) as total FROM transaction_item where item_id = '".$row->item_id."';");
                $total_penjualan = mysql_fetch_array($q_total);
                $val[] = $total_penjualan['total'];
